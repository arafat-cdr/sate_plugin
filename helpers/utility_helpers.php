...
if(!function_exists('wl_get_ip_address')){
	function wl_get_ip_address(){
...
# Get TimeZone By IP
if( !function_exists('wl_get_timeZone_by_ip') ){
	function wl_get_timeZone_by_ip( $ip ){
...
# conver a date time to another time
if( !function_exists("wt_convert_date_time_to_another_timezone") ){
	function wt_convert_date_time_to_another_timezone($wl_date_time, $format = "Y-m-d h:m:d", $timeZone = "Europe/Amsterdam" ){


		$date = new DateTime($wl_date_time.' +00');
		$date->setTimezone(new DateTimeZone($timeZone)); // +04

		$converted_date_time = $date->format($format);
		// $converted_date_time = $date->format($format)." timeZone: $timeZone";
...
if( !function_exists( 'wl_current_url' ) )
{
	function wl_current_url() {
